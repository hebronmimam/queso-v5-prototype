(() => {
  ...
  const SOURCE_URL = new URL("queso-cakes-catalog.js", SCRIPT_URL).href;
  const TAG_NAME = "queso-cakes-catalog";
  const NAVIGATION_EVENT = "queso-cakes-navigate";
  ...
  function patchElement(element) {
    ...
    root.querySelectorAll(".card-actions .button:not(.button--primary)").forEach((link) => {
      link.remove();
    });
    ...
    root.querySelectorAll("[data-primary-action]").forEach((link) => {
      ...
      if (link.dataset.quesoCatalogNavigationBound === "true") return;
      link.dataset.quesoCatalogNavigationBound = "true";
    ...
    root.querySelectorAll(".category-nav a").forEach((link, index) => {
      if (link.dataset.quesoCatalogNavigationBound === "true") return;
      link.dataset.quesoCatalogNavigationBound = "true";
  ...
  script.addEventListener(
    "error",
    () => {
      console.error("Could not load the Queso cakes catalog source.");
    },
    { once: true }
  );
