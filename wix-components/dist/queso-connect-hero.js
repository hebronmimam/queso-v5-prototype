(() => {
  ...
  const DEFAULT_IMAGE = new URL(
    "assets/generated-campaign/08-delivery-unboxing.png",
    REPO_ROOT
  ).href;
  ...
  class QuesoConnectHero extends HTMLElement {
    ...
    render() {
      const eyebrow = this.value("eyebrow", "Say hello");
      const title = this.value("title", "Let's talk cake.");
      const copy = this.value(
        "copy",
        "Need help with an order, delivery, ingredients or cake care? Send the details and Queso will point you in the right direction."
      );
      const buttonLabel = this.value("button-label", "Start a message");
      const buttonUrl = this.value("button-url", "#contact");
      const image = this.value("image", DEFAULT_IMAGE);
      const imageAlt = this.value(
        "image-alt",
        "Queso cheesecake delivery box"
      );
      const imagePosition = this.value("image-position", "center");
      const sticker = this.value("sticker", "We're listening");
      ...
      this.shadowRoot.innerHTML = `
        ...
        <section class="page-hero" aria-labelledby="queso-connect-title">
          <div class="page-hero-copy">
            <p class="eyebrow">${this.escape(eyebrow)}</p>
            <h1 id="queso-connect-title">${this.escape(title)}</h1>
