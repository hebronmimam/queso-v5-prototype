(() => {
  ...
  class QuesoSiteHeader extends HTMLElement {
    static get observedAttributes() {
      const attributes = [
        "announcement","home-url","cakes-label","cakes-url","flavors-label","flavors-url",
        "story-label","story-url","connect-label","connect-url","cart-label","cart-url","cart-count"
      ];
      for (let index = 1; index <= 4; index += 1) {
        attributes.push(
          `menu-${index}-title`,`menu-${index}-copy`,`menu-${index}-url`,
          `menu-${index}-image`,`menu-${index}-detail-image`
        );
      }
      return attributes;
    }
    ...
    render() {
      const announcement = this.value("announcement","Free island-wide delivery on orders over HKD 600");
      ...
      `).join("");
      const mobileLinks = items.map((item) => `<a href="${this.escape(item.url)}">${this.escape(item.title)}</a>`).join("");
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          *,*::before,*::after { box-sizing:border-box; }
          ...
          .header { position:relative; z-index:10; min-height:inherit; height:100%; background:var(--cream); border-bottom:2px solid var(--brown); }
          ...
          .marquee { display:flex; width:max-content; animation:header-marquee 28s linear infinite; will-change:transform; }
          .marquee:hover { animation-play-state:paused; }
          .marquee-row { display:flex; align-items:center; white-space:nowrap; font-size:11px; font-weight:800; letter-spacing:.02em; }
          .marquee-row span { padding:0 20px; }
          .marquee-row b { margin:0 18px; color:var(--yellow); font-size:17px; }
          @keyframes header-marquee { to { transform:translateX(-50%); } }
          ...
          .logo { width:148px; height:58px; object-fit:contain; }
          .desktop-nav { display:flex; gap:30px; margin-left:8px; font-size:12px; font-weight:800; letter-spacing:.04em; text-transform:uppercase; }
          .desktop-nav a { padding:9px 0; border-bottom:2px solid transparent; }
          .desktop-nav a:hover,.desktop-nav a:focus-visible { color:var(--orange); border-color:var(--orange); }
          .nav-tools { margin-left:auto; display:flex; align-items:center; gap:16px; }
          .cart-link,.menu-toggle { font-size:12px; font-weight:800; letter-spacing:.04em; text-transform:uppercase; }
          .cart-link:focus-visible,.menu-toggle:focus-visible,a:focus-visible { outline:3px solid var(--yellow); outline-offset:3px; }
          ...
          .cake-menu-links { display:grid; grid-template-columns:1fr 1fr; border-right:2px solid var(--brown); }
          .cake-menu-link { min-height:84px; padding:15px 24px; display:flex; align-items:center; justify-content:space-between; gap:20px; border-right:1px solid rgba(61,36,22,.35); border-bottom:1px solid rgba(61,36,22,.35); font-family:"Queso Lovelo",Arial,sans-serif; font-size:14px; text-transform:uppercase; }
          .cake-menu-link:nth-child(2n) { border-right:0; }
          .cake-menu-link:nth-last-child(-n+2) { border-bottom:0; }
          .cake-menu-link::after { content:"↗"; font-family:"Queso Quicksand",Arial,sans-serif; font-size:14px; }
          .cake-menu-link.is-active,.cake-menu-link:hover,.cake-menu-link:focus-visible { background:var(--orange); color:#fff; }

          .cake-menu-preview { display:grid; grid-template-columns:1fr 1fr 1.05fr; min-height:170px; background:#fff; }
          .cake-menu-preview img { width:100%; height:170px; object-fit:cover; border-right:2px solid var(--brown); }
          .cake-menu-preview__copy { padding:22px; display:flex; flex-direction:column; justify-content:center; }
          .cake-menu-preview__copy strong { font-family:"Queso Lovelo",Arial,sans-serif; font-size:24px; text-transform:uppercase; }
          .cake-menu-preview__copy p { margin:8px 0 0; font-size:12px; line-height:1.5; }

          .mobile-category-strip,.mobile-menu { display:none; }

          @media(max-width:1100px) {
            :host { min-height:170px; }
            .desktop-cake-menu { display:none; }
            .desktop-nav { display:none; }
            .menu-toggle { display:block; }
            .mobile-category-strip { display:flex; overflow-x:auto; scrollbar-width:none; border-top:1px solid rgba(61,36,22,.25); }
            .mobile-category-strip::-webkit-scrollbar { display:none; }
            .mobile-category-strip a { flex:0 0 auto; padding:12px 17px; border-right:1px solid rgba(61,36,22,.25); font-size:10px; font-weight:800; text-transform:uppercase; }
            .mobile-menu { position:absolute; inset:126px 0 auto; z-index:30; padding:20px var(--pad); background:var(--cream); border-bottom:2px solid var(--brown); }
            .mobile-menu.is-open { display:block; }
            .mobile-menu a { display:block; padding:15px 0; border-bottom:1px solid rgba(61,36,22,.25); font-family:"Queso Lovelo",Arial,sans-serif; font-size:22px; text-transform:uppercase; }
          }
        ...
        <header class="header">
          ...
          <div class="nav">
            <a href="${this.escape(this.value("home-url","/"))}" aria-label="Queso Bakehouse home"><img class="logo" src="${LOGO}" alt="Queso Bakehouse"></a>
          ...
          <div class="desktop-cake-menu" aria-label="Preview cake types">
            ...
            <div class="cake-menu-preview">
              ...
              <div class="cake-menu-preview__copy"><strong>${this.escape(active.title)}</strong><p>${this.escape(active.copy)}</p></div>
            </div>
          </div>
        </header>
      `;
    ...
    bindEvents() {
      ...
      cartLink?.addEventListener("click",() => {
        this.dispatchEvent(new CustomEvent("queso-cart-open",{
          bubbles:true,
          composed:true,
          cancelable:true,
