  ...
  class QuesoHero extends HTMLElement {
    static get observedAttributes() {
      ...
      for (let index = 1; index <= 3; index += 1) {
        attributes.push(
          `slide-${index}-eyebrow`,`slide-${index}-title`,`slide-${index}-copy`,
          `slide-${index}-image`,`slide-${index}-image-alt`,`slide-${index}-image-position`,
          `slide-${index}-primary-label`,`slide-${index}-primary-url`,
          `slide-${index}-secondary-label`,`slide-${index}-secondary-url`
        );
      }
      return attributes;
    }
    ...
    render() {
      ...
      const slideMarkup = slides.map((slide,index) => `
        <article class="hero-slide${index === this.activeSlide ? " is-active" : ""}" data-slide="${index}" aria-hidden="${index === this.activeSlide ? "false" : "true"}">
          ...
          <div class="hero-copy">
            ...
            ${index === 0 ? `<h1>${this.escape(slide.title)}</h1>` : `<h2>${this.escape(slide.title)}</h2>`}
      ...
      const dots = slides.map((_,index) => `<button class="hero-dot${index === this.activeSlide ? " is-active" : ""}" type="button" data-dot="${index}" aria-label="Show hero slide ${index + 1}" aria-pressed="${index === this.activeSlide ? "true" : "false"}"></button>`).join("");
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          *,*::before,*::after { box-sizing:border-box; }
          ...
          .hero { position:relative; width:100%; height:100%; min-height:650px; overflow:hidden; background:var(--brown); border-bottom:2px solid var(--brown); isolation:isolate; }
          .hero-slide { position:absolute; inset:0; opacity:0; visibility:hidden; pointer-events:none; transition:opacity 650ms ease,visibility 650ms ease; }
          .hero-slide.is-active { opacity:1; visibility:visible; pointer-events:auto; }
          .hero-slide__image { position:absolute; inset:0; display:block; width:100%; height:100%; object-fit:cover; background:var(--brown); }
          .hero-slide__overlay { position:absolute; inset:0; background:linear-gradient(90deg,rgba(253,243,230,.98) 0%,rgba(253,243,230,.88) 36%,rgba(253,243,230,.08) 67%); }
          .hero-copy { position:absolute; z-index:2; left:var(--pad); top:50%; width:min(640px,48vw); transform:translateY(-50%); }
          .eyebrow { margin:0 0 17px; color:#a84c09; font-size:12px; font-weight:850; letter-spacing:.13em; text-transform:uppercase; }
          h1,h2 { margin:0; font-family:"Queso Lovelo",Arial,sans-serif; font-size:clamp(64px,7.3vw,112px); font-weight:900; line-height:1.03; letter-spacing:0; text-transform:uppercase; }
          .lead { max-width:560px; margin:26px 0; font-size:clamp(16px,1.5vw,20px); line-height:1.55; }
          .actions { display:flex; flex-wrap:wrap; gap:10px; }

          .button { min-height:50px; padding:0 22px; display:inline-flex; align-items:center; justify-content:center; border:2px solid var(--brown); border-radius:10px; background:#fff; font-size:11px; font-weight:850; letter-spacing:.05em; text-transform:uppercase; transition:transform 150ms ease; }
          ...
          .hero-sticker { position:absolute; z-index:4; right:var(--pad); top:28px; padding:13px 21px; background:var(--pink); border:2px solid var(--brown); border-radius:999px; font-size:10px; font-weight:900; letter-spacing:.05em; transform:rotate(2deg); }
          .hero-controls { position:absolute; z-index:5; right:var(--pad); bottom:34px; display:flex; gap:8px; }
          .hero-dot { width:14px; height:14px; padding:0; border:2px solid #fff; border-radius:50%; background:transparent; cursor:pointer; }
          .hero-dot.is-active { background:var(--yellow); }

          @media(max-width:980px) { .hero-copy { width:min(600px,62vw); } }
          @media(max-width:680px) {
            :host,.hero { min-height:680px; }
            .hero-slide__overlay { background:linear-gradient(180deg,rgba(253,243,230,.98) 0%,rgba(253,243,230,.8) 46%,rgba(253,243,230,.05) 78%); }
            .hero-slide__image { object-position:62% center !important; }
            .hero-copy { left:20px; right:20px; top:44%; width:auto; }
            h1,h2 { font-size:55px; line-height:1.02; }
          ...
          @media(prefers-reduced-motion:reduce) { .hero-slide,.button { transition:none; } }
        </style>
        ...
        <section class="hero" aria-roledescription="carousel" aria-label="Featured Queso cheesecakes">
          ...
          ${this.bool("show-dots",true) ? `<div class="hero-controls" aria-label="Choose a hero slide">${dots}</div>` : ""}
        </section>
      `;
    }
    ...
    bindEvents() {
      ...
      hero?.addEventListener("touchstart",(event) => { this.touchStartX = event.changedTouches[0]?.screenX || 0; },{ passive:true });
